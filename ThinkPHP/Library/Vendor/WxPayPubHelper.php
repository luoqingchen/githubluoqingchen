...
class Common_util_pub
{
	...
	function postXmlSSLCurl($xml,$url,$second=30){
		...
		$sslcert_path = $_SERVER['DOCUMENT_ROOT'].c('SSLCERT_PATH');

		$sslkey_path = $_SERVER['DOCUMENT_ROOT'].c('SSLKEY_PATH');
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function createXml(){
	   	$this->parameters["appid"] = c('APPID');
	   	$this->parameters["mch_id"] = c('MCHID');
		...
		try
		{
          //检测必填参数
			if($this->parameters["re_openid"] == null ) {
				throw new SDKRuntimeException("裂变红包接口中，缺少必填参数openid！"."<br>");
			}elseif($this->parameters["total_amount"] == null){
				throw new SDKRuntimeException("退款申请接口中，缺少必填参数total_amount！"."<br>");
			}elseif($this->parameters["total_num"] == null){
				throw new SDKRuntimeException("裂变红包接口中，缺少必填参数total_num！"."<br>");
			}elseif ($this->parameters["amt_type"] == null) {
                throw new SDKRuntimeException("缺少发红包接口必填参数amt_type！"."<br>");
            }
			$this->parameters["wxappid"] = c('APPID');//公众账号ID
		   	$this->parameters["mch_id"] = c('MCHID');//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
		    return  $this->arrayToXml($this->parameters);
...
class Redpack_pub extends Wxpay_client_pub
{
	...
	function createXml()
	{
		try
		{
			//检测必填参数
			if($this->parameters["re_openid"] == null ) {
				throw new SDKRuntimeException("红包接口中，缺少必填参数openid！"."<br>");
			}elseif($this->parameters["total_amount"] == null){
				throw new SDKRuntimeException("退款申请接口中，缺少必填参数total_amount！"."<br>");
			}
		   	$this->parameters["wxappid"] = c('APPID');//公众账号ID
 ...
 * 获取红包记录
 */
  class Gethbinfo_pub extends Wxpay_client_pub
  {
	  function __construct() 
	{
		//设置接口链接
	...
	{		
		try 
		{
			...
			{
				throw new SDKRuntimeException("红包记录中，缺少必填参数mch_billno！"."<br>");
			}
		   	$this->parameters["appid"] = c('APPID');//公众账号ID
		   	$this->parameters["mch_id"] = c('MCHID');//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
